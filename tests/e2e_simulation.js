...
client.on('connect', async () => {
    ...
    const runSimulationSequence = async () => {
        ...
        for (let i = 0; i < 5; i++) {
            client.publish('envsense_pbl/telemetry/sim_device_001', JSON.stringify({
        ...
        client.publish('envsense_pbl/status/sim_device_001', JSON.stringify({
            state: "offline",
        ...
        for (let i = 0; i < 5; i++) {
            client.publish('envsense_pbl/telemetry/sim_device_002', JSON.stringify({
